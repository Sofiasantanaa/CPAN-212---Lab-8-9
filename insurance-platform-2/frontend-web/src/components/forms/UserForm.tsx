...
export default function UserForm({ initialData, isEdit = false }: UserFormProps) {
  ...
  return (
    ...
    >
      <h2 className="text-xl font-semibold">{isEdit ? 'Edit User' : 'Create User'}</h2>

      {/* FULL NAME */}
      <div>
        <label className="block text-sm font-medium">Full Name</label>
        <input {...register('fullName')} className="mt-1 w-full border rounded-lg px-3 py-2" />
      ...
      <div>
        ...
        <input {...register('email')} className="mt-1 w-full border rounded-lg px-3 py-2" />
          ...
          {roleOptions.map((role) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg border ${
                selectedRoles?.includes(role) ? 'bg-blue-600 text-white' : 'bg-gray-100'
              }`}
            >
              {role}
            </button>
      ...
      <div>
        ...
        <select {...register('status')} className="mt-1 w-full border rounded-lg px-3 py-2">
