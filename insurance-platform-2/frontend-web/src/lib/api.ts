...
export async function apiRequest<T>(
  ...
  if (!payload || typeof payload !== 'object' || !('success' in payload)) {
    throw new ApiRequestError('Invalid API response format', response.status, payload);
  }

  return payload as ApiSuccessResponse<T>;
}

//Functions that were not found and generate an error when calling them in UserForm.tsx
